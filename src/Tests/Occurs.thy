...
definition occurs_decl :: fun_decl
  where "occurs_decl \<equiv> ([aa, nn, xx], [cc,ii],
...
definition main_decl :: fun_decl
  where "main_decl \<equiv> ([], [],
    ...
    Callfun ''bb1'' ''occurs'' [(V aa), (V nn), (V ''foo'')];;
    Callfun ''bb2'' ''occurs'' [(V aa), (V nn), (V ''bar'')])"
...
definition proc_table :: program where "proc_table \<equiv> 
  (initial_proc(''main'' := (Some main_decl)))(''occurs'' := (Some occurs_decl))"
...
value "case interp proc_table init_state of Some (_,\<gamma>,\<mu>) \<Rightarrow> (\<gamma> ''bb1'', \<gamma> ''bb2'',\<mu>)"
