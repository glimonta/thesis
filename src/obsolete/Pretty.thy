theory Pretty
imports Eval Type_Checker "Show/Show_Instances"
begin
...
text \<open> We make a new instance of shows for the type @{term word}, in order to be able to
  ...
  instantiation Word.word :: (len)"show" begin
    definition "shows_prec p w \<equiv> shows_prec p (sint w)"

    definition "shows_list (l::'a Word.word list) \<equiv> showsp_list shows_prec 0 l"
...
\<close>
  definition showsp_mem :: "mem showsp" where
    "showsp_mem _ m \<equiv> shows_sep (showsp_block_at 0) shows_nl (List.enumerate 0 m)"
...
fun shows_type :: "type \<Rightarrow> shows" where
  "shows_type TInt = shows int_type"
| "shows_type (TPtr T) = shows_type T o shows '' *''"
| "shows_type (TNull) = shows ''void *''"

fun shows_type_space :: "type \<Rightarrow> shows" where
  "shows_type_space TInt = shows int_type o shows '' ''"
| "shows_type_space (TPtr T) = shows_type_space T o shows ''*''"
| "shows_type_space (TNull) = shows ''void *''"
...
text \<open>From the previous definitions on pretty printing the definition of @{term shows_exp}
  ...
  | "shows_exp (New T e) = (shows ''__MALLOC(sizeof('' o shows_type T o shows '') * '' o shows_paren (shows_exp e) o shows '')'')"
  | "shows_exp (Deref e) = shows ''*'' o shows_paren (shows_exp e)"
  | "shows_exp (Ref e) = (shows_unop ''&'' (shows_lexp e))"
  | "shows_exp (Index e1 e2) = shows_paren ( (shows_exp e1)) o shows CHR ''['' o shows_exp e2 o shows CHR '']''"
  | "shows_lexp (Derefl exp) = shows_paren (shows ''*'' o  shows_paren ((shows_exp exp)))"
  | "shows_lexp (Indexl e1 e2) = shows_paren ((shows_exp e1)) o shows CHR ''['' o shows_exp e2 o shows CHR '']''"
...
text \<open>From the previous definitions on pretty printing the definition of @{term shows_com}
  ...
  | "shows_com ind (Free x) =
      indent_basic ind ( shows ''free'' o shows_paren (shows_exp x))"
...
definition shows_vdecl :: "vdecl \<Rightarrow> shows" where
  "shows_vdecl \<equiv> \<lambda>(T,x). shows_type_space T o shows x"  

text \<open>Pretty printing of arguments of a function declaration are pretty printed as follows:
  <type> <argument_name>
\<close>
  definition shows_arg :: "vdecl \<Rightarrow> shows" where
    "shows_arg \<equiv> shows_vdecl"   
...
text \<open>A declaration of a local variable in a function is done with indentation = 1 and is
  ...
  <type> <argument_name>;
\<close>
  definition shows_local :: "vdecl \<Rightarrow> shows" where
    "shows_local vd \<equiv> indent_basic 1 (shows_vdecl vd)"

fun shows_rtype :: "type option \<Rightarrow> shows" where
  "shows_rtype None = shows ''void ''"
| "shows_rtype (Some T) = shows_type_space T"
...
text\<open> A function declaration is pretty printed by printing the return type, the name of the
  ...
  definition shows_fun_decl :: "fun_decl \<Rightarrow> shows" where
    "shows_fun_decl fdecl \<equiv> 
      shows_rtype (fun_decl.rtype fdecl) o shows_space o shows (fun_decl.name fdecl) o 
...
text \<open>A declaration of a global variable is done with indentation = 0 and is pretty printed
  ...
  <type> <argument_name>;
\<close>
  definition shows_global :: "vdecl \<Rightarrow> shows" where
    "shows_global s \<equiv> indent_basic 0 (shows_vdecl s)"
...
definition prepare_export :: "program \<hookrightarrow> string" where
  "prepare_export prog \<equiv> do {
    assert (valid_program prog) (EChecker EWf);
    assert (program_loc.tm_prog prog) (EChecker ETypeChecker);
    return (shows_prog prog '''')
  }"

interpretation program_loc p for p . (* TODO: Hack! This interpretation will cause trouble when 
  using program_loc after this point ... *)
