...
When performing a malloc call in a C program, there is a possibility that this call yields \verb|NULL|, in that case our semantics and the C semantics act different.
...
Therefore we have decided to assume an unlimited amount of memory and later when doing the translation process, we will wrap C's malloc function in an user defined macro that will check whether the malloc call is successful or not.
This way, what we will be able to guarantee about our generated program will either be generated and the execution of it in the semantics and the execution in the machine will yield final states that are equal or the program will abort when an out of memory error is encountered.
...
Note that when \textit{executing} this command in the CFG, following the node from a return node to a \verb|SKIP| node will pop the program pointer pointing to the \verb|SKIP| node and we will continue our ``execution'' in the location pointed by the next program pointer in the stack.
...
Note that, also in this case, when \textit{executing} this command in the CFG, following the node from a function call node to a \verb|SKIP| node will push a new program pointer to the stack that points to the location of the node that contains the command of the function and we will continue our ``execution'' in the location pointed by this next program pointer.
