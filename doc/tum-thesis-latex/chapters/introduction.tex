...
z:=x; x:=y; y:=z
\end{equation*}
Here ``$:=$'' is an assignment to a variable and ``$;$'' is the instruction sequencing.
Syntactically this is three statements separated by ``$;$'', where each statement is conformed by a variable, the symbol ``$:=$'' and a second variable.
...
\begin{comment}
  \begin{itemize}
    ...
    The meaning of a construct are modeled mathematically by objects that represent the effect of executing the construct.
...
It is partial correctness because it depends on the termination of the construct's execution.~\parencite{nipkow}
